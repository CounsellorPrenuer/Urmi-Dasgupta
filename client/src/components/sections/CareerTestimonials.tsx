...
export function CareerTestimonials() {
    ...
    useEffect(() => {
        const fetchTestimonials = async () => {
            try {
                const query = `*[_type == "testimonial" && category == "career"] { name, role, content, rating }`;
                const result = await sanityClient.fetch(query);
                console.log('Career Testimonials:', result);
    ...
    if (isLoading) {
        return (
            <section className="py-24 md:py-32 bg-muted/30 min-h-[300px] flex items-center justify-center">
                <div className="text-muted-foreground animate-pulse">Loading career testimonials...</div>
            </section>
        );
    }
    ...
    return (
        <section className="py-24 md:py-32 bg-muted/30" ref={ref} data-testid="section-career-testimonials">
            <div className="max-w-7xl mx-auto px-6">
                ...
                >
                    ...
                    <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                        Success stories from our career guidance program
                    </p>
                </motion.div>

                {testimonials.length === 0 ? (
                    <div className="text-center py-12">
                        <p className="text-muted-foreground">No career testimonials available at the moment.</p>
                    </div>
                ) : (
                    <motion.div
                        initial={{ opacity: 0 }}
